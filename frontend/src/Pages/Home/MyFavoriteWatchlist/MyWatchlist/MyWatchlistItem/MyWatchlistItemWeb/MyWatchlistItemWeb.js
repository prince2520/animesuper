...
const MyWatchlistItemWeb = ({res, index}) => {
    ...
    return (
        <div className="my-watchlist-table-item">
            <h5 style={{borderLeft: `${res.color}`}} className="item-no">
                {index + 1}
            </h5>
            <h5 className="item-img">
                <img alt='main_picture'
                     width="100%"
                     src={res.fields.img_url}/></h5>
            <h5 className="item-title">{res.fields.title}</h5>
            <div className="item-progress">
                <div className="progress-bar-detail">
                    <h5>{res.fields.progress_read_watched} / {res.fields.num_episode_or_chapter ? res.fields.num_episode_or_chapter : 'N/A'}</h5>
                </div>
            ...
            <h5 className="item-type">{res.fields.type.charAt(0).toUpperCase() + res.fields.type.slice(1)}</h5>
            <h5 className="item-delete cursor-btn"
                  onClick={() => {
            ...
            <h5 className="item-edit cursor-btn" onClick={() => {
